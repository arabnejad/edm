...
def test_export_creates_a_utf8_text_file(tmp_path: Path) -> None:
    ...
    saved_path = TabContentExporter().export_text(
...
def test_export_requires_confirmation_before_replacing_a_file(
    ...
    with pytest.raises(ExportTargetExistsError):
        exporter.export_text(TabExportRequest(target_path, "new"))

    assert target_path.read_text(encoding="utf-8") == "old"

    exporter.export_text(TabExportRequest(target_path, "new", overwrite=True))
...
def test_export_reports_invalid_destination_paths(
    ...
    with pytest.raises(TabExportError):
        TabContentExporter().export_text(TabExportRequest(target_path, "text"))
...
def test_export_rejects_a_parent_path_that_is_not_a_directory(
    ...
    with pytest.raises(TabExportError, match="not a directory"):
        TabContentExporter().export_text(
...
def test_open_failure_does_not_remove_a_file_the_export_did_not_create(
    ...
    with pytest.raises(TabExportError, match="denied"):
        TabContentExporter._create_new_file(target_path, "text")
...
def test_write_failure_removes_the_incomplete_new_file(
    ...
    with pytest.raises(TabExportError, match="disk full"):
        TabContentExporter._create_new_file(target_path, "text")
...
def test_failed_confirmed_replacement_keeps_the_old_file(
    ...
    target_path.write_text("old", encoding="utf-8")
    monkeypatch.setattr(
        "easy_docker_manager.tabs.tab_content_exporter.os.replace",
        Mock(side_effect=PermissionError("denied")),
    )

    with pytest.raises(TabExportError, match="denied"):
        TabContentExporter._replace_file(target_path, "new")
...
def test_failed_replacement_write_removes_its_temporary_file(
    ...
    failing_file.write.side_effect = OSError("disk full")
    monkeypatch.setattr(
        "easy_docker_manager.tabs.tab_content_exporter.tempfile.NamedTemporaryFile",
